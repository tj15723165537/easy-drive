...
import { deleteUser, getUserPage, UserPageParams, UserVO } from '@/api/modules/system'
...
const UserPage = () => {
  ...
  const columns: TableColumnsType<UserVO> = [
    ...
    { title: '更新时间', dataIndex: 'updated_at', align: 'center' },
    {
      ...
      dataIndex: 'status',
      align: 'center',
      render: (status) => <Tag color={status === 1 ? 'blue' : 'red'}>{status === 1 ? '正常' : '禁用'}</Tag>,
    },
    {
      title: '操作',
      align: 'center',
  ...
  const { data, loading, pagination, run, refresh } = usePagination<ResPage<UserVO>, [BaseParams]>(async (params) => {
    const searchParams: UserPageParams = form.getFieldsValue()
    const result = await getUserPage({ ...params, ...searchParams })
    return result.data!
  })
